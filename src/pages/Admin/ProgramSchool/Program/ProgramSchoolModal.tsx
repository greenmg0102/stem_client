...
import { programSchoolOrgCreate } from '../../../../api/admin/programSchool/program'

import 'swiper/css';
import 'swiper/css/navigation';
import 'swiper/css/pagination';

const ProgramSchoolModal = ({ typeModal, setTypeModal, setTypeResult }: { typeModal: boolean, setTypeModal: any, setTypeResult: any }) => {
    ...
    const saveType = async (isTrue: boolean) => {
        ...
        if (
            ...
            let result = await programSchoolOrgCreate(data)
                        ...
                        >
                            <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8 text-black dark:text-white-dark">
                                <div className="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 pt-6">
                                    <h5 className="font-bold text-lg text-gray-600">Please register <span className='font-bold text-gray-900 text-[20px]'>program school / org type</span>.</h5>
                                        ...
                                        <Tab.Panels>
                                            <Tab.Panel>
                                                <div className="active pt-5">
                                                    ...
                                                    <p className="">Program School / Org Street Address</p>
                                                    <input type="text" value={value.address} name="address" placeholder="Address..." className="form-input my-4" onChange={(e: any) => setValue({ ...value, [e.target.name]: e.target.value })} />

                                                    <p className="">Program School / Org City</p>
                                                    <input type="text" value={value.city} name="city" placeholder="City..." className="form-input my-4" onChange={(e: any) => setValue({ ...value, [e.target.name]: e.target.value })} />

                                                    <p className="">Program School / Org Zip Code</p>
                                                    <input type="text" value={value.zip} name="zip" placeholder="Zip code..." className="form-input my-4" onChange={(e: any) => setValue({ ...value, [e.target.name]: e.target.value })} />

                                                    <p className="">Neighborhood</p>
                                                    <input type="text" value={value.neighborhood} name="neighborhood" placeholder="Neighborhood..." className="form-input my-4" onChange={(e: any) => setValue({ ...value, [e.target.name]: e.target.value })} />
...
export default ProgramSchoolModal;